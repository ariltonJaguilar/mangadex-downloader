...
def save_images(
    image_data: list[tuple[int, bytes | Path]],
    output_dir: str | Path,
    chapter_name: str
) -> list[Path]:
    """
    ...
    Args:
        image_data: List of (index, image_bytes_or_path) tuples
    ...
    for idx, data in sorted(image_data, key=lambda x: x[0]):
        ...
        ext = data.suffix if isinstance(data, Path) else get_image_extension(data)
        filename = f"{idx:03d}{ext}"
        filepath = chapter_dir / filename
        
        if isinstance(data, Path):
            if data.resolve() != filepath.resolve():
