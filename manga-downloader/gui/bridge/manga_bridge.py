...
class FetchWorker(QThread):
    ...
    def __init__(self, url: str, headless: bool = True, language: str = "pt-br", fallback_english: bool = False):
        super().__init__()
        self.url = url
    ...
    def run(self):
        try:
            ...
            from src.api.providers import get_provider, resolve_title_url
            
            # Extract manga code
            source, manga_code = resolve_title_url(self.url)
            api = get_provider(source, self.language, fallback_english=self.fallback_english)
            
            # Fetch manga info
            manga = api.get_manga_info(manga_code, headless=self.headless)
            if not manga:
            ...
            manga_dict = {
                ...
                "title": manga.title,
                "author": manga.author,
                "selected_language": self.language if source == "mangadex" else "en",
            ...
            # Fetch chapters
            chapters = api.get_all_chapters(manga_code, headless=self.headless)
            chapters_list = []
            for ch in chapters:
...
class MangaBridge(QObject):
    ...
    def fetchManga(self, url: str):
        ...
        self._worker = FetchWorker(url, headless=headless, language=self._config_manager.get("mangadex_language"),
                                   fallback_english=self._config_manager.get("fallback_english", False))
